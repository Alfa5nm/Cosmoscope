...
function useDatasets(setDatasetError) {
  ...
  useEffect(() => {
    async function loadDatasets() {
      ...
      if (setDatasetError) setDatasetError(null);
      try {
        ...
        if (!data || !Array.isArray(data.datasets)) {
          throw new Error('Dataset catalog response malformed');
        }
        ...
        if (setDatasetError) {
          setDatasetError(error instanceof Error ? error.message : 'Unknown dataset load error');
        }
      }
    }
    loadDatasets();
  }, [setDatasets, setDatasetStatus, setDatasetError]);
}
    ...
    <div className="app-container">
      <header className="app-header">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          <span className={`status-pill ${datasetStatus}`}>
      ...
      <aside className="app-sidebar">
        {datasetStatus === 'error' && (
          <div className="banner banner-error">
            <strong>Cannot reach the Zoomverse API.</strong>
            <span>
              Start the Express backend (`cd backend && npm run dev`) so `/api/datasets` and NASA proxies are available.
            </span>
            {datasetError && <code>{datasetError}</code>}
          </div>
        )}
        <div>
      ...
      <main className="app-main">
        {datasetStatus === 'error' ? (
          <div className="map-fallback" role="alert">
            ...
            <p>
              The frontend could not reach the API gateway. Launch the backend so live NASA layers, annotations, and exports
              can load.
            </p>
          </div>
        ) : (
